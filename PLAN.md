...
- [x] Gemini 구조화 출력 (JSON Schema) 활용  ← **됨**(2026-09-16): LLM 은 Gemini 직결이 아니라 **OpenRouter(OpenAI 호환)** 이므로 *"게이트웨이가 지원하는 방식"* = `response_format={"type":"json_schema","strict":true}` 로 구현. 스키마 = `waynai-backend/src/main/resources/schema/travel_plan.schema.json`, 적용 지점 = 단일 도시 계획 생성(스트리밍 + 재생성 1회).
  - **측정**(억지로 만들지 않았다): ①OpenRouter 카탈로그(`GET /api/v1/models`, 443개) 에서 모델 체인 3종(`deepseek/deepseek-v3.2`·`deepseek/deepseek-chat`·`z-ai/glm-4.7-flash`) 이 **전부 `structured_outputs` 를 선언** ②**실제 스키마로 라이브 호출**: 비스트리밍 HTTP 200 + 최상위 16키가 스키마와 **정확히 일치**(공급자 Friendli), 스트리밍도 HTTP 200·델타 누적 4,386자가 스키마대로 파싱됨
  - ⚠️ **종전 `json_object` 는 "구문상 JSON" 만 강제**했고 키·타입은 프롬프트 문구에 의존했다. 이제 `strict` 라 스키마 밖 키가 나오지 않는다. 다만 **스키마는 의미를 보장하지 않는다** — 짧은 프롬프트로 시험했을 때 `days=0`·1일치 일정이 나왔고, 일수 정합은 여전히 `reconcileDays` 가 잡는다
  - ⚠️ **osh-ai-gateway 경유(`OPENROUTER_BASE_URL`) 는 이 자리에서 측정하지 못했다**(`.25` 접속 금지). 그래서 라우터가 **거부를 실증한 뒤 강등**한다: 4xx/501 + `response_format|json_schema|structured` 오류면 같은 모델로 스키마 없이 1회 재시도 → 성공하면 "스키마가 원인" 으로 확정하고 이후 `json_object` 로 보내며 `warn` 을 남긴다. 수동 차단은 `OPENROUTER_STRUCTURED_OUTPUTS=false`
  - 가드: `TravelPlanSchemaTest`(strict 규격 + DTO 정합 + **검사기 변이 4종**) · `OpenRouterResponseFormatTest`(바디 구성 + **강등 오탐 7건**) · `GeminiApiClientSchemaTest`(이름→스키마 배선)
  - ⚠️ 권역분할(segment) 경로는 스키마를 쓰지 않는다 — 출력 모양이 `itinerary` 조각이라 스키마가 다르다(별건)
...
- [x] 예산 대비 경비 비교  ← **됨**(2026-09-16): `BudgetAdvisorService` + `BudgetAssessmentDto`. 총액 산출(이미 있던 `computeCosts`) 위에 **사용자 예산과의 비교**를 얹는다 — `WITHIN`(여유 5%↑) · `TIGHT`(초과는 아니나 빠듯) · `OVER` · `UNKNOWN`(예산 미언급). 계획에 `budgetAssessment` 로 붙고 SSE `budget` 이벤트로도 나간다. 프런트는 `StreamResult.vue` 예산 패널.
  - 예산 입력: `?budgetKrw=` 파라미터 우선, 없으면 **문장에서 코드로 추출**(`BudgetParser` — "예산 100만원", "1인당 50만원", "30만원 이내"). 의도 분석 LLM 은 `budgetLevel`("저렴/보통/고급")만 뽑아 금액 비교에 못 쓴다
  - ★ **오탐이 더 위험해서 그쪽을 먼저 잠갔다** — "2박3일"·"4인"·"오전 10시"·"3만보 걷기" 를 예산으로 읽으면 멀쩡한 계획에 "예산 초과" 라는 **틀린 판정**이 붙는다. `'원' 없는 단위 숫자는 예산 낱말이 곁에 있을 때만` · `금액이 여럿인데 어느 것이 예산인지 모르면 포기(로그 남김)` 로 막았고, 규칙을 빼 보니 실제로 "3만보" 가 300,000원으로 잡혔다(변이 확인)
  - ⚠️ 원화만 읽는다(USD·JPY 표기는 미지원)
- [x] 경비 절감 제안 ("이 호텔 대신 에어비앤비를 쓰면 20% 절감")  ← **됨**(2026-09-16): 항목별 규칙 6종(숙소 −30% · 식비 −25% · 현지교통 −40%(국내)/−30%(해외) · 입장료 −30% · 항공 −15% · 마지막 수단으로 일정 −1일). 절감액 큰 순으로 **부족분을 메울 만큼만** 고르고, 다 적용해도 모자라면 모자란다고 말한다(`stillOver`).
  - ⚠️ **LLM 을 부르지 않는다**(요구사항). 계획 생성 경로에 동기로 끼는 단계라 모델을 부르면 비용·지연이 그대로 더해지고, 절감액이 회차마다 흔들린다. `computeCosts` 가 이미 만든 항목별 금액에 **산술만** 한다
  - ★ 절감률은 추정이므로 제안마다 `basis`("식비 160,000원의 25%")를 함께 실어 화면에 "근거 … (추정)" 으로 표시한다
  - ★ **'일정 하루 줄이기' 는 절감액이 가장 크지만 여행 자체를 줄이는 것**이라 정렬에서 빼고, 다른 제안으로 부족할 때만 맨 뒤에 붙인다. 이미 싼 숙소(1박 6만원 이하×물가계수)에는 "등급 낮추기" 를 권하지 않는다
  - 가드: `BudgetAdvisorServiceTest`(12) · `BudgetParserTest`(10, 음성 5) · **`TravelOrchestratorBudgetWiringTest`(4 — 배선)**. 배선 테스트는 "로직은 맞는데 안 불린다" 를 잡으려고 넣었고, 실제로 `setBudgetAssessment` 한 줄을 빼니 4/4 실패·이벤트 타입만 바꿔도 실패했다
