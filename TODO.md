...
## 5. 인증/로그인 정책 — ✅ **2026-09-16 결정: 익명 토큰 유지**

**로그인을 넣지 않는다.** 09-05 에 도입한 익명 소유자 토큰(`X-Owner-Token`, SHA-256 해시만 저장)이
이미 동작 중이고, 로그인을 넣는 순간 **개인정보 처리자**가 된다 — 처리방침·보관기간이 따라온다.

🔴 **대가는 알고 받는다**: 토큰을 잃으면 서버 저장분도 복구할 수 없다. 그래서 ①UI 에 코드 표시·복사·
경고를 넣었고(`/travel-plan` "다른 기기에서 보기") ②일일 백업 대상에 넣었다. **백업이 유일한 안전망**이다.

⇒ 아래는 **결정 당시의 선택지 기록**이다. 되살리려면 위 판단부터 다시 봐야 한다.
